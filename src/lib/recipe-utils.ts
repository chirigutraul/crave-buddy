...
/**
 * Generates a markdown formatted string from a weekly meal plan
 */
export const generateWeeklyPlanMarkdown = (params: {
  weekName: string;
  weekMeals: any;
  ...
  days.forEach((day) => {
    ...
    if (dayTotal > 0) {
      dailyMeals += `- **Daily Total:** ${dayTotal} kcal\n`;
    }
    dailyMeals += "\n";
  });

  const weeklyMaintenanceCalories = dailyMaintenanceCalories * 7;
  const totalWeeklyDeficit = weeklyMaintenanceCalories - weeklyTotals.calories;

  const markdown = `# ${weekName}
...
- **Calories:** ${weeklyTotals.calories.toLocaleString()} kcal
- **Protein:** ${weeklyTotals.protein}g
- **Carbohydrates:** ${weeklyTotals.carbohydrates}g
- **Fat:** ${weeklyTotals.fat}g
- **Fiber:** ${weeklyTotals.fiber}g

### Daily Averages

- **Calories:** ${averageDailyCalories.toLocaleString()} kcal
- **Protein:** ${Math.round((weeklyTotals.protein / 7) * 10) / 10}g
- **Carbohydrates:** ${Math.round((weeklyTotals.carbohydrates / 7) * 10) / 10}g
- **Fat:** ${Math.round((weeklyTotals.fat / 7) * 10) / 10}g
- **Fiber:** ${Math.round((weeklyTotals.fiber / 7) * 10) / 10}g

### Calorie Balance

- **Weekly Maintenance Calories:** ${weeklyMaintenanceCalories.toLocaleString()} kcal
- **Total Weekly Calories:** ${weeklyTotals.calories.toLocaleString()} kcal
- **${
    totalWeeklyDeficit >= 0 ? "Total Weekly Deficit" : "Total Weekly Surplus"
  }:** ${Math.abs(totalWeeklyDeficit).toLocaleString()} kcal
  - ${
    totalWeeklyDeficit >= 0 ? "Average deficit" : "Average surplus"
  } per day: ${Math.round(Math.abs(totalWeeklyDeficit) / 7)} kcal
...
/**
 * Downloads a weekly plan as a markdown file
 */
export const downloadWeeklyPlanMarkdownFile = (
  weekName: string,
  markdown: string
): void => {
  const blob = new Blob([markdown], { type: "text/markdown;charset=utf-8" });
  ...
  const link = document.createElement("a");
  link.href = url;
  link.download = `${weekName.replace(/[^a-z0-9]/gi, "-").toLowerCase()}.md`;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
  URL.revokeObjectURL(url);
